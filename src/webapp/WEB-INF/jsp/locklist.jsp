...
<tr>
	<td class="recent"><a href="<jmwiki:link value="${lock.topicName}" />"><c:out value="${lock.topicName}"/></a></td>
	<td class="recent"><f:formatDate value="${lock.time}" type="both" dateStyle="MEDIUM" timeStyle="MEDIUM" /></td>
	<td><a href="<jmwiki:link value="Special:Unlock" />?topic=<jmwiki:encode value="${lock.topicName}" />"><f:message key="locklist.unlock"/></a></td>
</tr>
		</c:forEach>
</table>
	</c:otherwise>
